...
import ingenias.exception.NotFound;
  ...
  public class BolsaIVentaAccionesDefaultCommControl extends DefaultCommControl{
  ...
  public BolsaIVentaAccionesDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("AVentaAccionesB");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("IUAVentaAcciones") &&
    	 sb.isState("IUAVentaAcciones")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"AVentaAccionesB").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("IUAVentaAcciones");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"AVentaAccionesB");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("AVentaAccionesB");
	       lockProcessed("AVentaAccionesB");
           //MainInteractionManager.log("Removing lock AVentaAccionesB",this.getAgent().getLocalName()+"-"+sb.getConversationID());
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for IURVentaAcciones")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("IURVentaAcciones")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for IURVentaAcciones");           
    	   //try {

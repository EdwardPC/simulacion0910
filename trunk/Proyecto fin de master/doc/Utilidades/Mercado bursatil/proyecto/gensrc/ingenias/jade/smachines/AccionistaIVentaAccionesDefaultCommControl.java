...
import ingenias.exception.NotFound;
  ...
  public class AccionistaIVentaAccionesDefaultCommControl extends DefaultCommControl{
  ...
  public AccionistaIVentaAccionesDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("RVentaAccionesA");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("IURVentaAcciones") &&
    	 sb.isState("IURVentaAcciones")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"RVentaAccionesA").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("IURVentaAcciones");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"RVentaAccionesA");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("RVentaAccionesA");
	       lockProcessed("RVentaAccionesA");
           //MainInteractionManager.log("Removing lock RVentaAccionesA",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("IUAVentaAcciones");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for IUAVentaAcciones")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("IUAVentaAcciones")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for IUAVentaAcciones");           
    	   //try {
		   ...
		   futureStates.add("endIUAVentaAcciones");

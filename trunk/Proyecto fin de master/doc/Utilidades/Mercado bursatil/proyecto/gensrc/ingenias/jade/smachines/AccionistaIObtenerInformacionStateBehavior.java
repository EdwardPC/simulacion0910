...
public class AccionistaIObtenerInformacionStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public AccionistaIObtenerInformacionStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "IURObtenerInformacion");
			    
			    
			      // Receiving a message    
			      smf.add("IUAObtenerInformacion", "waiting for IUAObtenerInformacion");
			      
			      // Next states after receiving "IUAObtenerInformacion"
			      
			       smf.add("waiting for IUAObtenerInformacion","endIUAObtenerInformacion");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("IURObtenerInformacion", "IUAObtenerInformacion");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
  	try {  	  
      String[] options=new String[]{"IURObtenerInformacion"};
      AID[] actors=null;
      Vector<AID> actorsv=new Vector<AID>();
      Vector<String> rolesv=new Vector<String>();
      
      actorsv.addAll(this.getActor("Bolsa"));
      {      
       Vector<AID> receivers=this.getActor("Bolsa");      
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","IObtenerInformacion",completelistInvolvedActors);
  ...
  if (this.isState("IUAObtenerInformacion") ) {  // State changed by other agent and upted
    //in the parent class
         
      Vector options=new Vector();
      
      options.add("endIUAObtenerInformacion");
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"IUAObtenerInformacion",
                   "IObtenerInformacion",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("IUAObtenerInformacion");
      this.addState("waiting for IUAObtenerInformacion");
      this.notifyStateTransitionExecuted("IUAObtenerInformacion", "waiting for IUAObtenerInformacion");
  } 
  ...
  if (this.isState("IURObtenerInformacion")) {
     ...
     try {
      ...
      options.add("IUAObtenerInformacion");      
      
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("IURObtenerInformacion",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "IURObtenerInformacion","IObtenerInformacion",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("IURObtenerInformacion", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("IURObtenerInformacion", "ABORTED");
    	  } else  {
  ...
  if (this.isState("endIUAObtenerInformacion")) {

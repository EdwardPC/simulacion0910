...
public class EmpresaIRegistroEmpresaStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public EmpresaIRegistroEmpresaStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "IURRegistro");
			    
			    
			      // Receiving a message    
			      smf.add("IUARegistro", "waiting for IUARegistro");
			      
			      // Next states after receiving "IUARegistro"
			      
			       smf.add("waiting for IUARegistro","endIUARegistro");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("IURRegistro", "IUARegistro");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
  	try {  	  
      String[] options=new String[]{"IURRegistro"};
      AID[] actors=null;
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","IRegistroEmpresa",completelistInvolvedActors);
  ...
  if (this.isState("IUARegistro") ) {  // State changed by other agent and upted
    //in the parent class
         
      Vector options=new Vector();
      
      options.add("endIUARegistro");
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"IUARegistro",
                   "IRegistroEmpresa",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("IUARegistro");
      this.addState("waiting for IUARegistro");
      this.notifyStateTransitionExecuted("IUARegistro", "waiting for IUARegistro");
  } 
  ...
  if (this.isState("IURRegistro")) {
     ...
     try {
      ...
      options.add("IUARegistro");      
      
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("IURRegistro",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "IURRegistro","IRegistroEmpresa",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("IURRegistro", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("IURRegistro", "ABORTED");
    	  } else  {
  ...
  if (this.isState("endIUARegistro")) {

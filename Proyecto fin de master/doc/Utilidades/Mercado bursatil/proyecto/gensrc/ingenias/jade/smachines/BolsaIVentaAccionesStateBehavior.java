...
public class BolsaIVentaAccionesStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public BolsaIVentaAccionesStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","IURVentaAcciones");
			    
			    
			    
			      // Receiving a message    
			      smf.add("IURVentaAcciones", "waiting for IURVentaAcciones");
			      
			      // Next states after receiving "IURVentaAcciones"
			      
			       smf.add("waiting for IURVentaAcciones","IUAVentaAcciones");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("IUAVentaAcciones", "endIUAVentaAcciones");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "IVentaAcciones","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("IURVentaAcciones") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"IURVentaAcciones",
                   "IVentaAcciones",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("IURVentaAcciones");
      this.addState("waiting for IURVentaAcciones");
      this.notifyStateTransitionExecuted("IURVentaAcciones", "waiting for IURVentaAcciones");
  } 
  ...
  if (this.isState("IUAVentaAcciones")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("IUAVentaAcciones",optionsA,this)){
           ...
           "IUAVentaAcciones","IVentaAcciones",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("IUAVentaAcciones", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("IUAVentaAcciones", "ABORTED");
    	  } else  {

...
import ingenias.exception.NotFound;
  ...
  public class BolsaIObtenerInformacionDefaultCommControl extends DefaultCommControl{
  ...
  public BolsaIObtenerInformacionDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("AObtenerInformacionB");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("AObtenerInformacionB");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("IUAObtenerInformacion") &&
    	 sb.isState("IUAObtenerInformacion")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"AObtenerInformacionB").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("IUAObtenerInformacion");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"AObtenerInformacionB");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("AObtenerInformacionB");
	       lockProcessed("AObtenerInformacionB");
           //MainInteractionManager.log("Removing lock AObtenerInformacionB",this.getAgent().getLocalName()+"-"+sb.getConversationID());
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for IURObtenerInformacion")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("IURObtenerInformacion")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for IURObtenerInformacion");           
    	   //try {

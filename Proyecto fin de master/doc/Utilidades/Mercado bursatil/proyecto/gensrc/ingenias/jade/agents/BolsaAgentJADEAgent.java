...
public class BolsaAgentJADEAgent
		 extends JADEAgent {         
 
		 public BolsaAgentJADEAgent(){
		 super(new BolsaAgentProtocol(),new BolsaAgentInteractionLocks());
		 }
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
		...
		expectedInput=this.getMSM().getMentalEntityByType("ACompraAccionesB");
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
			    ...
			    addExpectedInputs(tobject, "ACompraAccionesB","1",expectedInput);
             	addConsumedInput(to,"ACompraAccionesB",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist || expectedInput.size()!=0;
		} 
	      
		expectedInput=this.getMSM().getMentalEntityByType("AObtenerInformacionB");
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
				nonExistingInputs.add("AObtenerInformacionB");
			 } else {
			    addExpectedInputs(tobject, "AObtenerInformacionB","1",expectedInput);
             	addConsumedInput(to,"AObtenerInformacionB",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist || expectedInput.size()!=0;
		} 
	      
		expectedInput=this.getMSM().getMentalEntityByType("AVentaAccionesB");
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
				nonExistingInputs.add("AVentaAccionesB");
			 } else {
			    addExpectedInputs(tobject, "AVentaAccionesB","1",expectedInput);
             	addConsumedInput(to,"AVentaAccionesB",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist || expectedInput.size()!=0;
		} 
	      
		expectedInput=this.getMSM().getMentalEntityByType("ARegistroEmpresaB");
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
				nonExistingInputs.add("ARegistroEmpresaB");
			 } else {
			    addExpectedInputs(tobject, "ARegistroEmpresaB","1",expectedInput);
             	addConsumedInput(to,"ARegistroEmpresaB",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist || expectedInput.size()!=0;
		} 
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	...
  	   	boolean correctRole=conversation.getPlayedRole().equals ("Bolsa");
       	...
       	if (tobject.getType().equals("TPRRegistroB") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
		...
		boolean allEntitiesExist=true;
	     	...
	     	expectedInput=this.getMSM().obtainConversationalMentalEntityByType(conversation,"EstadoBolsa");
			if (expectedInput.size()==0 && !("1".equals("0..n")))
				nonExistingInputs.add("EstadoBolsa");
			else
	      ...
	      allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
             ...
             expectedApp=(ingenias.jade.components.Application)getAM().getApplication("BolsaApp");
             tobject.addApplication("BolsaApp",expectedApp);
        ...
        /*     
	     ...
	     boolean alreadyExists=true;
		    ...
		    {ARegistroEmpresaB expectedOutputARegistroEmpresaB=		    
		     new ARegistroEmpresaB(MentalStateManager.generateMentalEntityID());			
             to.add(new OutputEntity(expectedOutputARegistroEmpresaB,TaskOperations.CreateWF));
            }
		...
		if (validConversationType){
       	...
       	if (tobject.getType().equals("TPRCompraAccionesB") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
        ...
        /*     
	     ...
	     boolean alreadyExists=true;
 			...
 			{
 			ACompraAccionesB expectedOutputACompraAccionesB=
 				new ACompraAccionesB(MentalStateManager.generateMentalEntityID());
 			if (RuntimeConversation.class.isAssignableFrom(expectedOutputACompraAccionesB.getClass())){
 			    java.lang.reflect.Method m;
				try {
					m = expectedOutputACompraAccionesB.getClass().getMethod("setInteraction", new Class[]{Interaction.class});
					m.invoke(expectedOutputACompraAccionesB, new Interaction("")) ;	  
            ...
            to.add(new OutputEntity(expectedOutputACompraAccionesB,TaskOperations.CreateMS));
            }
		...
		if (validConversationType){
       	...
       	if (tobject.getType().equals("TPRObtenerInformacionB") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
        ...
        /*     
	     ...
	     boolean alreadyExists=true;
 			...
 			{
 			AObtenerInformacionB expectedOutputAObtenerInformacionB=
 				new AObtenerInformacionB(MentalStateManager.generateMentalEntityID());
 			if (RuntimeConversation.class.isAssignableFrom(expectedOutputAObtenerInformacionB.getClass())){
 			    java.lang.reflect.Method m;
				try {
					m = expectedOutputAObtenerInformacionB.getClass().getMethod("setInteraction", new Class[]{Interaction.class});
					m.invoke(expectedOutputAObtenerInformacionB, new Interaction("")) ;	  
				} catch (SecurityException e) {
            ...
            to.add(new OutputEntity(expectedOutputAObtenerInformacionB,TaskOperations.CreateMS));
            }
		...
		if (validConversationType){
       	...
       	if (tobject.getType().equals("TPRVentaAccionesB") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
        ...
        /*     
	     ...
	     boolean alreadyExists=true;
 			...
 			{
 			AVentaAccionesB expectedOutputAVentaAccionesB=
 				new AVentaAccionesB(MentalStateManager.generateMentalEntityID());
 			if (RuntimeConversation.class.isAssignableFrom(expectedOutputAVentaAccionesB.getClass())){
 			    java.lang.reflect.Method m;
				try {
					m = expectedOutputAVentaAccionesB.getClass().getMethod("setInteraction", new Class[]{Interaction.class});
					m.invoke(expectedOutputAVentaAccionesB, new Interaction("")) ;	  
				} catch (SecurityException e) {
            ...
            to.add(new OutputEntity(expectedOutputAVentaAccionesB,TaskOperations.CreateMS));
            }
         ...
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new TPRRegistroBTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
         ...
         if (goalname.equals("Colaborar")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				boolean canbescheduled=false;
				for (int k=0;k<conversations.size();k++){
					tobject=new TPRCompraAccionesBTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal Colaborar",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
         ...
         if (goalname.equals("Colaborar")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				boolean canbescheduled=false;
				for (int k=0;k<conversations.size();k++){
					tobject=new TPRObtenerInformacionBTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal Colaborar",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
         ...
         if (goalname.equals("Colaborar")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				boolean canbescheduled=false;
				for (int k=0;k<conversations.size();k++){
					tobject=new TPRVentaAccionesBTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal Colaborar",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("TPRRegistroB");					
         
         
                  
         
                   
         ttypes.add("TPRCompraAccionesB");					
         
         
                  
         
                   
         ttypes.add("TPRObtenerInformacionB");					
         
         
                  
         
                   
         ttypes.add("TPRVentaAccionesB");					
   ...
   /* */
	...
	Vector evetns=null;
     ...
     app=BolsaAppInit.createInstance(this);
	 //app.registerOwner(this);
	 	    
     this.getAM().addApplication("BolsaApp",app);        
	 events=new Vector();
	 actions=new Vector();

	 if (getGraphics()!=null)
	  getGraphics().addApplication("BolsaApp", events,actions);    

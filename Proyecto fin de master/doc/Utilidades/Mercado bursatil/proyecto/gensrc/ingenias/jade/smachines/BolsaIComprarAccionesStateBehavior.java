...
public class BolsaIComprarAccionesStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public BolsaIComprarAccionesStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","IURCompraAcciones");
			    
			    
			    
			      // Receiving a message    
			      smf.add("IURCompraAcciones", "waiting for IURCompraAcciones");
			      
			      // Next states after receiving "IURCompraAcciones"
			      
			       smf.add("waiting for IURCompraAcciones","IUACompraAcciones");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("IUACompraAcciones", "endIUACompraAcciones");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "IComprarAcciones","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("IURCompraAcciones") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"IURCompraAcciones",
                   "IComprarAcciones",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("IURCompraAcciones");
      this.addState("waiting for IURCompraAcciones");
      this.notifyStateTransitionExecuted("IURCompraAcciones", "waiting for IURCompraAcciones");
  } 
  ...
  if (this.isState("IUACompraAcciones")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("IUACompraAcciones",optionsA,this)){
           ...
           "IUACompraAcciones","IComprarAcciones",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("IUACompraAcciones", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("IUACompraAcciones", "ABORTED");
    	  } else  {

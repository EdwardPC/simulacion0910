...
public class AccionistaIComprarAccionesStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public AccionistaIComprarAccionesStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "IURCompraAcciones");
			    
			    
			      // Receiving a message    
			      smf.add("IUACompraAcciones", "waiting for IUACompraAcciones");
			      
			      // Next states after receiving "IUACompraAcciones"
			      
			       smf.add("waiting for IUACompraAcciones","endIUACompraAcciones");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("IURCompraAcciones", "IUACompraAcciones");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
  	try {  	  
      String[] options=new String[]{"IURCompraAcciones"};
      AID[] actors=null;
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","IComprarAcciones",completelistInvolvedActors);
  ...
  if (this.isState("IUACompraAcciones") ) {  // State changed by other agent and upted
    //in the parent class
         
      Vector options=new Vector();
      
      options.add("endIUACompraAcciones");
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"IUACompraAcciones",
                   "IComprarAcciones",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("IUACompraAcciones");
      this.addState("waiting for IUACompraAcciones");
      this.notifyStateTransitionExecuted("IUACompraAcciones", "waiting for IUACompraAcciones");
  } 
  ...
  if (this.isState("IURCompraAcciones")) {
     ...
     try {
      ...
      options.add("IUACompraAcciones");      
      
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("IURCompraAcciones",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "IURCompraAcciones","IComprarAcciones",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("IURCompraAcciones", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("IURCompraAcciones", "ABORTED");
    	  } else  {
  ...
  if (this.isState("endIUACompraAcciones")) {

...
public class BolsaIObtenerInformacionStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public BolsaIObtenerInformacionStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","IURObtenerInformacion");
			    
			    
			    
			      // Receiving a message    
			      smf.add("IURObtenerInformacion", "waiting for IURObtenerInformacion");
			      
			      // Next states after receiving "IURObtenerInformacion"
			      
			       smf.add("waiting for IURObtenerInformacion","IUAObtenerInformacion");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("IUAObtenerInformacion", "endIUAObtenerInformacion");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "IObtenerInformacion","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("IURObtenerInformacion") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if ("1".equals("n")){
         try{
          Vector<AID> receivers=this.getActor("Accionista");    
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"IURObtenerInformacion",
                   "IObtenerInformacion",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("IURObtenerInformacion");
      this.addState("waiting for IURObtenerInformacion");
      this.notifyStateTransitionExecuted("IURObtenerInformacion", "waiting for IURObtenerInformacion");
  } 
  ...
  if (this.isState("IUAObtenerInformacion")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("IUAObtenerInformacion",optionsA,this)){
           ...
           "IUAObtenerInformacion","IObtenerInformacion",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("IUAObtenerInformacion", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("IUAObtenerInformacion", "ABORTED");
    	  } else  {

...
public class AccionistaIVentaAccionesStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public AccionistaIVentaAccionesStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "IURVentaAcciones");
			    
			    
			      // Receiving a message    
			      smf.add("IUAVentaAcciones", "waiting for IUAVentaAcciones");
			      
			      // Next states after receiving "IUAVentaAcciones"
			      
			       smf.add("waiting for IUAVentaAcciones","endIUAVentaAcciones");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("IURVentaAcciones", "IUAVentaAcciones");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
  	try {  	  
      String[] options=new String[]{"IURVentaAcciones"};
      AID[] actors=null;
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","IVentaAcciones",completelistInvolvedActors);
  ...
  if (this.isState("IUAVentaAcciones") ) {  // State changed by other agent and upted
    //in the parent class
         
      Vector options=new Vector();
      
      options.add("endIUAVentaAcciones");
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"IUAVentaAcciones",
                   "IVentaAcciones",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("IUAVentaAcciones");
      this.addState("waiting for IUAVentaAcciones");
      this.notifyStateTransitionExecuted("IUAVentaAcciones", "waiting for IUAVentaAcciones");
  } 
  ...
  if (this.isState("IURVentaAcciones")) {
     ...
     try {
      ...
      options.add("IUAVentaAcciones");      
      
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("IURVentaAcciones",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "IURVentaAcciones","IVentaAcciones",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("IURVentaAcciones", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("IURVentaAcciones", "ABORTED");
    	  } else  {
  ...
  if (this.isState("endIUAVentaAcciones")) {

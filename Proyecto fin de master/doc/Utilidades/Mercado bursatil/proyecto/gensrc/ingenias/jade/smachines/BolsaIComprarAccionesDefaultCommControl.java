...
import ingenias.exception.NotFound;
  ...
  public class BolsaIComprarAccionesDefaultCommControl extends DefaultCommControl{
  ...
  public BolsaIComprarAccionesDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("ACompraAccionesB");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("IUACompraAcciones") &&
    	 sb.isState("IUACompraAcciones")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"ACompraAccionesB").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("IUACompraAcciones");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"ACompraAccionesB");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("ACompraAccionesB");
	       lockProcessed("ACompraAccionesB");
           //MainInteractionManager.log("Removing lock ACompraAccionesB",this.getAgent().getLocalName()+"-"+sb.getConversationID());
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for IURCompraAcciones")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("IURCompraAcciones")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for IURCompraAcciones");           
    	   //try {

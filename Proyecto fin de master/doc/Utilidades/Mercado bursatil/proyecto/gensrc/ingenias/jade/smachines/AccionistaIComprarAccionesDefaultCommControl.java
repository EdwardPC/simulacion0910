...
import ingenias.exception.NotFound;
  ...
  public class AccionistaIComprarAccionesDefaultCommControl extends DefaultCommControl{
  ...
  public AccionistaIComprarAccionesDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("RCompraAccionesA");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("IURCompraAcciones") &&
    	 sb.isState("IURCompraAcciones")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"RCompraAccionesA").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("IURCompraAcciones");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"RCompraAccionesA");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("RCompraAccionesA");
	       lockProcessed("RCompraAccionesA");
           //MainInteractionManager.log("Removing lock RCompraAccionesA",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("IUACompraAcciones");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for IUACompraAcciones")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("IUACompraAcciones")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for IUACompraAcciones");           
    	   //try {
		   ...
		   futureStates.add("endIUACompraAcciones");

...
import ingenias.exception.NotFound;
  ...
  public class EmpresaIRegistroEmpresaDefaultCommControl extends DefaultCommControl{
  ...
  public EmpresaIRegistroEmpresaDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("RRegistroEmpresaE");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("IURRegistro") &&
    	 sb.isState("IURRegistro")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"RRegistroEmpresaE").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("IURRegistro");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"RRegistroEmpresaE");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("RRegistroEmpresaE");
	       lockProcessed("RRegistroEmpresaE");
           //MainInteractionManager.log("Removing lock RRegistroEmpresaE",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("IUARegistro");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for IUARegistro")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("IUARegistro")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for IUARegistro");           
    	   //try {
		   ...
		   futureStates.add("endIUARegistro");

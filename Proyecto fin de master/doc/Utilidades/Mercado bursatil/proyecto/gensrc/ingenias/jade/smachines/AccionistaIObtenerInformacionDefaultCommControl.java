...
import ingenias.exception.NotFound;
  ...
  public class AccionistaIObtenerInformacionDefaultCommControl extends DefaultCommControl{
  ...
  public AccionistaIObtenerInformacionDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("RObtenerInformacionA");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("RObtenerInformacionA");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("IURObtenerInformacion") &&
    	 sb.isState("IURObtenerInformacion")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"RObtenerInformacionA").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("IURObtenerInformacion");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"RObtenerInformacionA");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("RObtenerInformacionA");
	       lockProcessed("RObtenerInformacionA");
           //MainInteractionManager.log("Removing lock RObtenerInformacionA",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("IUAObtenerInformacion");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for IUAObtenerInformacion")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("IUAObtenerInformacion")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for IUAObtenerInformacion");           
    	   //try {
		   ...
		   futureStates.add("endIUAObtenerInformacion");

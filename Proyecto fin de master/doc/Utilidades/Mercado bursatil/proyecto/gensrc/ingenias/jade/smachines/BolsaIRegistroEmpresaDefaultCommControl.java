...
import ingenias.exception.NotFound;
  ...
  public class BolsaIRegistroEmpresaDefaultCommControl extends DefaultCommControl{
  ...
  public BolsaIRegistroEmpresaDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("ARegistroEmpresaB");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("IUARegistro") &&
    	 sb.isState("IUARegistro")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"ARegistroEmpresaB").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("IUARegistro");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"ARegistroEmpresaB");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("ARegistroEmpresaB");
	       lockProcessed("ARegistroEmpresaB");
           //MainInteractionManager.log("Removing lock ARegistroEmpresaB",this.getAgent().getLocalName()+"-"+sb.getConversationID());
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for IURRegistro")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("IURRegistro")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for IURRegistro");           
    	   //try {

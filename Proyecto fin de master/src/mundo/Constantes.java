package mundo;

public class Constantes {

	public static final String ARRIBA = "Arriba";
	public static final String ABAJO = "Abajo";
	public static final String DERECHA = "Derecha";
	public static final String IZQUIERDA = "Izquierda";
	public static final String AUTOVIA_D_1 = "Autovia_D_1";
	public static final String AUTOVIA_D_2 = "Autovia_D_2";
	public static final String AUTOVIA_I_1 = "Autovia_I_1";
	public static final String AUTOVIA_I_2 = "Autovia_I_2";

	public static final String SECUNDARIA_D = "Secundaria_D";
	public static final String SECUNDARIA_I = "Secundaria_I";
	
	public static final String CARRIL_ENTRADA = "Carril_entrada";
	
	public static final String CARRIL_SALIDA = "Carril_salida";
	
	public static final String CALLE_PRINCIPAL_1 = "Calle_principal_1";
	public static final String CALLE_PRINCIPAL_2 = "Calle_principal_2";
	
	public static final String CALLE_1 = "Calle_1";
	public static final String CALLE_2 = "Calle_2";
	
	public static final String CALLEJON = "Callejon";
	
	public static final String CRUCE = "Cruce";
	public static final String EDIFICIO = "Edificio";
	public static final String CAMPO = "Campo";
	public static final String TIERRA = "Tierra";
	public static final String STOP = "Stop";
	public static final String CEDA_EL_PASO = "Ceda";
	public static final String SEMAFORO = "Semaforo";
	public static final String AUTOMOVIL = "Automovil";
	public static final String CAMION = "Camion";
	public static final String MOTOCICLETA = "Motocicleta";
	public static final String EMERGENCIA = "Emergencia";
	public static final String BORDE = "Borde";
	public static final  String SEPARACION = "Separacion";
	
	public static final  String VERDE = "Verde";
	public static final  String AMARILLO = "Amarillo";
	public static final  String ROJO = "Rojo";
	
}
